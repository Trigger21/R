...
```{r}
emp[emp$JOB_ID != 'ST_CLERK' & emp$JOB_ID != 'SH_CLERK' & emp$JOB_ID !='SA_REP',
...
[문제44] oracle에서 null(알수없는 값, 적용할수 없는, 할당할수 없는 값)
         R에서 NA(결측치)를 체크하는 함수는? 초기값 없는 변수 
...
## 3-5. 정렬
   ...
   - order를 사용하면 행을 제한하는 조건을 사용불가 
     행을 조건절로 만든 변수를 다른 변수에 담아놓고 order 수행 
...
[문제56] 30번 부서 사원들의 last_name, salary 를 출력하세요.
         단, salary를 기준으로 내림차순정렬 하세요.
```{r}

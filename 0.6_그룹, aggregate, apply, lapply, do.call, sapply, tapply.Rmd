...
```
  ...
  2) rowSums() : 배열, 행렬, 데이터 프레임의 행의 합
  3) rowMeans() : 배열, 행렬, 데이터 프레임의 행의 평
...
```
  ...
  4) colSums() : 배열, 행렬, 데이터 프레임의 열의 합
  5) colMeans() : 배열, 행렬, 데이터 프레임의 열의 평
...
```
 ...
 - 선생님 풀이 
 1. apply 결과(행의 합)를 Total column 저장과 동시에 cbind해서 저장
 2. apply 결과(열의 합)를 'Total'을 name column 넣어주면서 rbind해서 저장
```{r}
